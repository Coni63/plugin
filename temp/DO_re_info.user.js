...
// @version        4.02
// @updateURL		https://sites.google.com/site/coni63do/plugin/DO_re_info.user.js
// @downloadURL		https://sites.google.com/site/coni63do/plugin/DO_re_info.user.js
// @installURL		https://sites.google.com/site/coni63do/plugin/DO_re_info.user.js
...
function read_RC(){
	if(Type[0].innerHTML=='Rapport d\'espionnage - Défense'){
		for (i = 0, c = Temporaire.length; i < c; i=i+2) {
			...
			Temporaire[i+1].innerHTML=Convert(DelPoint(Temporaire[i+1].innerHTML));
			IndiceJavaScript++;
			}
		...
		var Money = parseInt(line[1].lastChild.innerHTML.replace(/[^0-9]/gi,''));
		var Gold = parseInt(line[2].lastChild.innerHTML.replace(/[^0-9]/gi,''));
		var Petrole = parseInt(line[3].lastChild.innerHTML.replace(/[^0-9]/gi,''));
		var Muni = parseInt(line[4].lastChild.innerHTML.replace(/[^0-9]/gi,''));
		var Kero = parseInt(line[5].lastChild.innerHTML.replace(/[^0-9]/gi,''));
		var Diesel = parseInt(line[6].lastChild.innerHTML.replace(/[^0-9]/gi,''));
		var total = Money + Gold * 1000 + Petrole * 500 + Muni * 7 + Kero * 10003 + Diesel * 5002;
		
		line[1].lastChild.innerHTML=Convert(Money);
		line[2].lastChild.innerHTML=Convert(Gold);
		line[3].lastChild.innerHTML=Convert(Petrole);
		line[4].lastChild.innerHTML=Convert(Muni);
		line[5].lastChild.innerHTML=Convert(Kero);
		line[6].lastChild.innerHTML=Convert(Diesel);
...
function generate_RC_info(){
	...
	Add.innerHTML +='<table class="battleReport" width="100%">'
		...
		+ '<tr>'
			...
			+ '<td style="text-align:right">' + Convert(Argent_Unites) + '</td>'
		+ '</tr>'
		+ '<tr>'
			+ '<td style="text-align:left">Entretiens horaire</td>' 
			+ '<td style="text-align:right">' + Convert(Entretien)+ '</td>'
		+ '</tr>'
		+'</table>';
}
...
function Add_list_opponent(){
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv3); 
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv4); 
...
function Add_DYOA(){
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv5); 
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv6);
...
function add_result(Valeur_unit_att_init, Valeur_unit_def_init, Valeur_unit_att_perdu, Valeur_unit_def_perdu, entretien_pre_att, entretien_post_att, entretien_pre_def, entretien_post_def, Qte_muni,diesel_att, kero_att, id_loc){
	...
	info_rc2.innerHTML='<tr>'
		...
		+ '</tr>'
		+ '<tr>'
			+ '<td style="text-align:left">' + Convert(Valeur_unit_att_init) + '</td>'
			+ '<td style="text-align:right">' + Convert(Valeur_unit_def_init) + '</td>'
		+ '</tr>'
		+ '<tr>'
			+ '<th colspan="2" style="text-align:center">'
		...
		+ '</tr>'
		+ '<tr>'
			+ '<td style="text-align:left">' + Convert(Valeur_unit_att_perdu) + '</td>' 
			+ '<td style="text-align:right">' + Convert(Valeur_unit_def_perdu) + '</td>'
		+ '</tr>'
		+ '<tr>'
		+ '<th colspan="2" style="text-align:center">'
	...
	+ '</tr>'
	+ '<tr>'
		+ '<td style="text-align:left">' + Convert(entretien_pre_att)+'/h' + '</td>'
		+ '<td style="text-align:right">' + Convert(entretien_pre_def)+'/h' + '</td>'
	+ '</tr>'
	+ '<tr>'
		+ '<th colspan="2" style="text-align:center">'
	...
	+'</tr>'
	+ '<tr>'
		+ '<td style="text-align:left">' + Convert(entretien_post_att)+'/h' + '</td>'
		+ '<td style="text-align:right">' + Convert(entretien_post_def)+'/h' + '</td>'
	+ '</tr>'
		+ '<tr>'
			+ '<th colspan="2" style="text-align:center">'
				+'Munitions'
		...
		+ '</tr>'
		+ '<tr>'
			+ '<td style="text-align:left">' + Convert(diesel_att * 5002 + kero_att * 10003 + Qte_muni * 7 + Valeur_unit_att_init) + '</td>'
			+ '<td style="text-align:right">' + Convert(Valeur_unit_def_init) + '</td>'
		+ '</tr>'
		+ '<tr>'
			+ '<th colspan="2" style="text-align:center">'
		...
		+ '</tr>'
		+ '<tr>'
			+ '<td style="text-align:left">' + Convert(diesel_att * 5002 + kero_att * 10003 + Qte_muni * 7 + Valeur_unit_att_perdu) + '</td>'
			+ '<td style="text-align:right">' + Convert(Valeur_unit_def_perdu) + '</td>'
		+ '</tr>';
...
function Add_option(){
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv7); 
...
function simuler(armee_att, id_armee){
	...
	cout_ress = Qte_muni * 7 + diesel_att * 5002 + kero_att * 10003;
...
function optimize(){
	...
	document.getElementById('show_result').innerHTML = 'Temps : '+elapsedTime/1000+' s</br>Nombre d\'armées simulées : '+armee.length+' sur '+total+' armées possibles</br>Nombre d\'unités possibles : '+Unit_selectionnable.length;
}
...
document.getElementById('report').firstChild.firstChild.appendChild(MyDiv9); 
...
document.getElementById('report').firstChild.firstChild.appendChild(MyDiv10);
...
function Add_stat_simu(){
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv11); 
	...
	document.getElementById('report').firstChild.firstChild.appendChild(MyDiv12);
