...
var modeC = ['', '',' k',' m',' M',' b',' B',' C',' kC',' mC',' MC',' bC',' BC',' C²',' kC²',' mC²',' MC²',' bC²',' BC²',' C<sup>3</sup>'];
var classique = ['', '',' mil.',' Mil.',' Md.',' bl.',' Bd.',' tl.',' Td.',' ql.',' Qd.',' qil.',' Qid.',' sl.',' Sd.',' spl.',' Spd.',' ol.',' Od.',' nl.'];

	var nb = nombre.split('.');
	var Power = nb.length-1;
	nombre = (nb[0]+'.'+nb[1]).replace(/[*a-z]/gi,'');
...
function Create_Aff(nb){
	...
	var mode_mis = document.getElementById('type_conv_hidden').innerHTML;
...
var classique = ['',' mil.',' Mil.',' Md.',' bl.',' Bd.',' tl.',' Td.',' ql.',' Qd.',' qil.',' Qid.',' sl.',' Sd.',' spl.',' Spd.',' ol.',' Od.',' nl.'];
	...
	//if (!mode){mode=1;};
	if (nombre < 0 ){var sign = true} else { var sign = false };
	nombre = Math.abs(nombre);
	if(nombre >= Math.pow(10,6)){
		var Power = (Math.log(nombre)/Math.LN10).toFixed();
		Power = (Power - Power % 3 - 3);
		nombre /= Math.pow(10, Power);
		nombre = lisibilite_nombre(Arrondir(nombre, 0));
		if( mode == 0){
			nombre+= modeC[Power/3];
		} else if (mode == 1){
			nombre+=' * 10<sup>'+Power+'</sup>';
		}else if(mode == 2){
			nombre+= classique[Power/3];
		}
		if (sign == true){nombre='- '+nombre;}
	}
	return nombre;
}
