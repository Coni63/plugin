...
function tiers(a){
	...
	flag[a]=false;
	inp_txt[a][0].value=lisibilite_nombre(DelPoint(Stock[a])/3);
}

function demi(a){
	a=parseInt(a.replace(/[^0-9]/gi,''));
	flag[a]=false;
	inp_txt[a][0].value=lisibilite_nombre(DelPoint(Stock[a])/2);
}
...
function calcul_all(){
	for (var j=0; j<inp_txt.length; j++){
		if(flag[j]==true){
			...
			inp_txt[j][0].value = lisibilite_nombre(new Big(total).str());
		}
	}
}
