...
function Lire_MP(){
	...
	GM_xmlhttpRequest({
	  method: "GET",
	  url: "http://game.desert-operations.fr/"+World+"/postfach.php?mode=1&folder=3",
	  synchronous:true,
	  ...
	  onload: function(response){
		...
		for (var i = 0 ; i<EachMail.length ;i++){
			if (EachMail[i].firstChild.firstChild.firstChild.src.split('/')[7]=="email_open_image.png"){
				...
				GM_xmlhttpRequest({
				  ...
				  data: 'mail='+id_MP+'&imgPrfx=images%2Fclassic&folder=3',
				  synchronous:true,
				  headers: {"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"},
				  onload: function(response) {
		...
		dhtmlx.message("Tous les MP du commerces sont lus");
