...
// @downloadURL    http://dorercbdd.esy.es/script/new/DO_all_in_one.user.js
// @installURL 	   http://dorercbdd.esy.es/script/new/DO_all_in_one.user.js
// @updateURL	   http://dorercbdd.esy.es/script/new/DO_all_in_one.user.js
// @require		   http://dorercbdd.esy.es/script/new/fonctions.js
// @require		   http://dorercbdd.esy.es/script/new/bignumber.js
// @require		   http://dorercbdd.esy.es/script/new/unite.js
...
// @resource logo  http://dorercbdd.esy.es/script/new/alerte.png
...
function Display_attaque_bot(){
	...
	if (etat == true){
		...
		var Chkbx_army = document.getElementsByName('choice_army')
		
		Chkbx_army[0].onchange = function(){
			if(armee1==false){
				GM_setValue('Armee1', true);
				armee1 = true;
			}else{
				GM_setValue('Armee1', false);
				armee1 = false;
			}
		}
		
		Chkbx_army[1].onchange = function(){
			if(armee2==false){
				GM_setValue('Armee2', true);
				armee2 = true;
			}else{
				GM_setValue('Armee2', false);
				armee2 = false;
			}
		}
		
		Chkbx_army[2].onchange = function(){
			if(armee3==false){
				GM_setValue('Armee3', true);
				armee3 = true;
			}else{
				GM_setValue('Armee3', false);
				armee3 = false;
			}
		}
				...
				if (mode =='1'){
					...
					for (var i = 0 ; i< allTID.length ; i++){
						if (allTID[i].value == Trade_id){
							allTID[i].parentNode.parentNode.style.cssText="background-color:#CCCCCC";
						}
					}
				}
				setTimeout(function(){Achat_List(JSON_array, mode);},12000);
			  }
			});
		} else {
...
function DO_commerce_color(){
	...
	for (var i = 0 ; i<span.length ; i++){
		...
		//span[0].parentNode.parentNode.style.cssText += 'font-weight:bold;  border:1px solid #AFA';
...
function DO_versement(){
		...
		var position = document.getElementsByClassName('formRow color3 alignCenter')[0];
		...
		var Btn1 = document.createElement('input');
			...
			Btn1.id="distrib_pet";
			Btn1.onclick = function(){
				var json = document.getElementsByClassName('allianceAllocateMoney')[0].getAttribute('rel');
				...
				GM_xmlhttpRequest({
				  ...
				  onload: function(response){
					...
					dhtmlx.message("Le pétrole a été versé à l'alliance");
				  }
				});
			}
			...
			Btn2.onclick = function(){
				...
				GM_xmlhttpRequest({
				  ...
				  onload: function(response){
					...
					dhtmlx.message("Les munitions ont été versées à l'alliance");
				  }
				});				
			}
...
function DO_RC_RE_info(){
	...
	function add_result(Valeur_unit_att_init, Valeur_unit_def_init, Valeur_unit_att_perdu, Valeur_unit_def_perdu, entretien_pre_att, entretien_post_att, entretien_pre_def, entretien_post_def, Qte_muni,diesel_att, kero_att, id_loc){
		...
		var titre5=document.createElement('div');
		titre5.className='battleUnitsHeader attacker';
		titre5.innerHTML='<div class="battleUnitsHeaderIcon buildings"></div><div class="battleUnitsHeaderLabel"><div class="headlineCenter">RC Infos : </div></div>';
	...
	if (Path =='battleReport.php'){
		...
		document.getElementById('battleAnimationPlayBox').remove();
	
		var tableau = document.getElementsByTagName('tbody');
		var Temporaire_att = tableau[1].getElementsByTagName('tr');
		var Temporaire_def = tableau[2].getElementsByTagName('tr');
		var Temporaire_bat = tableau[3].getElementsByTagName('tr');

		var posi_plugin = document.getElementById('battleOverview');
		...
		function read_RC(){
			...
			for (var i = 1 ; i < Temporaire_bat.length ; i++){
				...
				Bat_Debut[i] = Temporaire_bat[i].firstChild.nextSibling.innerHTML.replace(/[^0-9]/gi,'');
				Bat_fin[i] = Temporaire_bat[i].firstChild.nextSibling.nextSibling.innerHTML.replace(/[^0-9]/gi,'');
				Bat_perdu[i] = Temporaire_bat[i].firstChild.nextSibling.nextSibling.nextSibling.innerHTML.replace(/[^0-9]/gi,'');
			}
		}
		...
		var global= document.createElement('div');
			global.id='global';
			global.className='battleUnitsHeader attacker';
			global.innerHTML='<div class="battleUnitsHeaderIcon buildings"></div><div class="battleUnitsHeaderLabel"><div class="headlineCenter">Simulateurs : </div></div>';

		posi_plugin.appendChild(global); 
	...
	if (Path=='spionageReport.php'){
		...
		var posi_plugin = document.getElementsByClassName('spionageReport')[0];
		...
		function read_RE(){
			...
			if(Type[0].innerHTML=='Rapport d\'espionnage - Unités espions'){
				var Cout = 0;
				var Def = 0;
				var Info = new Array();
					Info['Satellite espion'] = new Array(5000000,15000); 
					Info['Drone'] = new Array(1000000,2500); 
					Info['Avion de reconnaissance'] = new Array(1400000,8000); 
					Info['Agent'] = new Array(300000,1000); 
				
				for (i = 0, c = Temporaire.length; i < c; i=i+2) {
					Unites_Nom[IndiceJavaScript] = Temporaire[i].innerHTML;
					Qte[IndiceJavaScript] = DelPoint(Temporaire[i+1].innerHTML);
					Temporaire[i+1].innerHTML=Create_Aff(Temporaire[i+1].innerHTML,type_conversion);
					Cout += Qte[IndiceJavaScript]*Info[Unites_Nom[IndiceJavaScript]][0];
					Def += Qte[IndiceJavaScript]*Info[Unites_Nom[IndiceJavaScript]][1];
					IndiceJavaScript++;
				}
				var nb100 = Def /22500;
				var gold = nb100*10000;
				
				Cout=new Big(Cout).str();
				gold=new Big(gold).str();
				nb100=new Big(nb100).str();
				
				document.getElementsByTagName('table')[0].innerHTML +='<table class="battleReport" width="100%">'
				+ '<tr>'
					+ '<th colspan="3">'
						+'<h2 class="battleReport" style="text-align:center">Valeurs espionnages</h2>'
					+ '</th>'
				+ '</tr>'
				+ '<tr>'
					+ '<td style="text-align:left">Valeur des unités en $</td>'
					+ '<td style="text-align:right">' + Create_Aff(Cout,type_conversion) + '</td>'
				+ '</tr>'
				+ '<tr>'
					+ '<td style="text-align:left">Nb de sats pour passer à 100 %</td>' 
					+ '<td style="text-align:right">' + Create_Aff(nb100,type_conversion)+ '</td>'
				+ '</tr>'
				+ '<tr>'
					+ '<td style="text-align:left">Or nécessaire</td>' 
					+ '<td style="text-align:right">' + Create_Aff(gold,type_conversion)+ '</td>'
				+ '</tr>'
				+'</table>';
				
			}
			if(Type[0].innerHTML=='Rapport d\'espionnage - Dépôt'){
				var col2 = document.getElementsByClassName('rightAlign');
				for (var i = 0 ; i < col2.length ; i++){
					col2[i].innerHTML = Create_Aff(col2[i].innerHTML, type_conversion);
				}
			}
			if(Type[0].innerHTML=='Rapport d\'espionnage - Construction'){
				var col2 = document.getElementsByClassName('rightAlign');
				for (var i = 0 ; i < col2.length ; i++){
					col2[i].innerHTML = Create_Aff(col2[i].innerHTML, type_conversion);
				}
...
function Display_Read_MP(){
	var liste_der = document.createElement('div');
		liste_der.className = "infopanel";
		liste_der.innerHTML = '<a class="toggleInfopanelTab up" rel="do_read_mp" href="#"><strong>DO Lire MP</strong></a><div id="do_read_mp" class="infoTab up ltr" style="display: none;"><input type="button" value="Lire MP - Commerce" id="read_mp_comm"/><input type="button" value="Lire MP - Boite de reception" id="read_mp_inbox"/><div id="nb_mp"></div></div>'
		
		document.getElementById('infopanel_inner').insertBefore(liste_der, document.getElementById('infopanelBoardLink'));

		document.getElementById('read_mp_comm').onclick = function(){Lire_MP(3);}
		document.getElementById('read_mp_inbox').onclick = function(){Lire_MP(5);}
}

function Lire_MP(num_folder){
	
	GM_xmlhttpRequest({
	  method: "GET",
	  url: "http://game.desert-operations.fr/"+World+"/postfach.php?mode=1&folder="+num_folder,
	  synchronous:true,
	  headers: {"Content-Type": "application/x-www-form-urlencoded"},
	  onload: function(response){
		var MyDom = parseAJAX_ResponseHTML(response);
		var EachMail = MyDom.getElementById('inboxTable').childNodes[2].childNodes;
		var count = 0;
		for (var i = 0 ; i<EachMail.length ;i++){
			if (EachMail[i].firstChild.firstChild.firstChild.src.split('/')[7]=="email_open_image.png"){
				var Unread_Json = EachMail[i].firstChild.childNodes[2].firstChild.getAttribute('rel');
				var id_MP = JSON.parse(Unread_Json).sMailId;
				count++;
				GM_xmlhttpRequest({
				  method: "POST",
				  url: "http://game.desert-operations.fr/"+World+"/Webservices/getMail.php",
				  data: 'mail='+id_MP+'&imgPrfx=images%2Fclassic&folder='+num_folder,
				  synchronous:true,
				  headers: {"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"},
				  onload: function(response) {}
				});	
		...
		dhtmlx.message("Tous les MP sont lus");
		document.getElementById('nb_mp').innerHTML=count+" MP ont été lus !"; 
	  }
	});
...
if (Path!='bank.php' && Path!='allianceadministration.php'){
	Add_label_val();
}
if (document.getElementById('infopanel_inner')!=null){
	document.getElementsByClassName('infopanel last')[0].remove();
	Display_conversion();
	Display_attaque_bot();
	DO_reco_display();
	Display_safe_att();
...
if (Path=='handel.php'){
	if (document.getElementById('tradeGraph')!=null){
		document.getElementById('tradeGraph').remove();
	}
}
